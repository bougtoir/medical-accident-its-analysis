#!/usr/bin/env python3
"""Create Japanese manuscript for BMJ Open submission.

Same content as EN version but with all text in Japanese.
BMJ Open format: structured abstract (Objectives/Design/Setting/Participants/
...
r = p.add_run('キーワード: ')
r.bold = True
p.add_run('疼痛管理; 鎮痛薬; 生態学的研究; 文化的要因; 日本; 処方')
...
abs_sections = [
    ('目的:', (
        '疼痛関連処方が日本47都道府県間で変動するかを検討し、'
        '「日本人患者は忍耐強い」という単一の文化的ラベルが'
        '鎮痛薬の必要性を一律に予測するという仮定を検証する。'
    )),
    ('デザイン:', (
        '全数行政レセプトデータを用いた横断的生態学的研究。'
    )),
    ('セッティング:', (
        '日本全47都道府県。NDBオープンデータ第10回（2023年4月〜2024年3月）を使用。'
    )),
    ('参加者:', (
        '日本の国民皆保険制度の約1億2500万人をカバーする'
        '都道府県レベル集計データ。個人レベルデータは使用していない。'
    )),
    ('主要・副次アウトカム:', (
        '主要アウトカム：急性周術期鎮痛薬処方／手術指数。'
        '副次アウトカム：外来神経障害性疼痛薬処方／手術指数'
        '（慢性術後痛プロキシ、交絡疾患調整あり・なし）。'
    )),
    ('結果:', (
...
# 本研究の強みと限界
# ============================================================
add_heading_text('本研究の強みと限界', level=2)

sl_items = [
    '約1億2500万人をカバーする全数レセプトデータによりサンプリングバイアスを排除。',
    '周術期デザインにより、Phase 1の全患者が入院手術患者であるため'
    '医療アクセスが交絡因子として中立化される。',
    '同一データソースの疾患特異的薬剤プロキシを用いた透明かつ再現可能な'
    'データベース内交絡調整フレームワーク。',
    '分析単位は都道府県（n=47）であり個人ではないため、'
    '生態学的相関が個人レベルの関連を反映しない可能性がある（生態学的誤謬）。',
    'NDBオープンデータには診断コードがなく、神経障害性疼痛薬プロキシは'
    '全適応症を捕捉するため、慢性術後痛に特異的ではない。',
]
...
d1 = (
    f'第一に、日本の文化的忍耐にもかかわらず、{cite(5,6)}'
...
doc.add_paragraph(
    f'第二に、神経障害性疼痛処方の東北超過（未調整d={unadj_d:.2f}）は交絡疾患で大部分説明された。'
    f'糖尿病薬でr=0.87の相関を示し、調整後に東北効果は{attenuation:.0f}%減弱し非有意となった。'
)

d3 = (
    f'第三に、Phase 1（急性）とPhase 2（慢性、調整後）の正相関（r=0.29, P=0.052）は'
    f'急性疼痛管理の地域パターンが慢性疼痛負担に影響する可能性を示唆する。'
    f'Kehlet et alが示した個人レベルの知見と一致する。{cite(23)}'
)
p = doc.add_paragraph()
add_ref_runs(p, d3)
...
doc.add_paragraph(
    ...
    '透明な交絡調整手法、周術期デザインによる医療アクセス中立化、'
    '他国で再現可能なデータベース内交絡調整フレームワークである。'
...
doc.add_paragraph(
    ...
    'TOは本研究の全責任を負い、データへのアクセスを有し、出版の決定を管理した。'
)

doc.add_paragraph()
add_heading_text('資金', level=1)
doc.add_paragraph('本研究は公的、商業的、または非営利セクターのいかなる助成機関からも特定の助成を受けていない。')
...
outpath = os.path.join(BMJOPEN_DIR, 'BMJOpen_manuscript_JA.docx')
doc.save(outpath)
