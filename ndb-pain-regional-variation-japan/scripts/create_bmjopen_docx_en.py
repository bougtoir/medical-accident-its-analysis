#!/usr/bin/env python3
"""Create English manuscript for BMJ Open.

BMJ Open Original Research requirements:
- Abstract: structured (Objectives, Design, Setting, Participants,
  Primary and secondary outcome measures, Results, Conclusions) ≤300 words
- Body: ≤4,000 words (excl. abstract/refs/legends/tables)
- "Strengths and limitations of this study" box (3–5 bullets) after abstract
- References: Vancouver style — superscript [n] in order of first appearance
- Figures: uploaded as separate TIFF/EPS files (not embedded); legends at end
...
- Contributor and guarantor statement
...
# Word count and article info
p = doc.add_paragraph()
r = p.add_run('Article type: ')
r.bold = True
p.add_run('Original Research')
...
r.bold = True
p.add_run('pain management; analgesics; ecological study; cultural factors; Japan; prescriptions')

doc.add_page_break()

# ============================================================
# ABSTRACT (Objectives, Design, Setting, Participants,
#           Primary and secondary outcome measures, Results, Conclusions)
#           ≤300 words
# ============================================================
add_heading_text('Abstract', level=1)

abstract_objectives = (
    'To determine whether pain-related prescribing varies across '
    'Japan\u2019s 47 prefectures, testing the assumption that a single cultural label '
    '\u2014\u201cJapanese patients are stoic\u201d\u2014predicts analgesic need uniformly.'
)

abstract_design = (
    'Cross-sectional ecological study using population-complete administrative claims data.'
)

abstract_setting = (
    'All 47 prefectures in Japan, using the National Database of Health Insurance Claims '
    '(NDB Open Data, 10th edition; April 2023\u2013March 2024).'
)

abstract_participants = (
    'Prefecture-level aggregate data covering approximately 125 million insured individuals '
    'under Japan\u2019s universal health insurance system. No individual-level data were used.'
)

abstract_outcomes = (
    'Primary outcome: acute perioperative analgesic prescribing per surgery '
    '(inpatient analgesic prescription units divided by surgical procedure count). '
    'Secondary outcome: outpatient neuropathic pain drug prescribing per surgery as a '
    'chronic postsurgical pain proxy, with and without adjustment for confounding diseases.'
)
...
abstract_results = (
    ...
    f'Standardised claim ratios confirmed {scr_analgesic_ratio:.2f}-fold variation after '
    f'age-sex standardisation.'
)
...
abstract_sections = [
    ('Objectives', abstract_objectives),
    ('Design', abstract_design),
    ('Setting', abstract_setting),
    ('Participants', abstract_participants),
    ('Primary and secondary outcome measures', abstract_outcomes),
...
abstract_total = sum(wc(t) for t in [
    abstract_objectives, abstract_design, abstract_setting,
    abstract_participants, abstract_outcomes, abstract_results,
    abstract_conclusions,
])
print(f'Abstract word count: {abstract_total} (BMJ Open limit: 300)')

doc.add_page_break()

# ============================================================
# STRENGTHS AND LIMITATIONS OF THIS STUDY (3–5 bullets)
# ============================================================
add_heading_text('Strengths and limitations of this study', level=2)

sl_items = [
    'Population-complete data from Japan\u2019s National Database of Health Insurance Claims '
    'covering approximately 125 million insured individuals eliminates sampling bias.',

    'The perioperative design neutralises healthcare access as a confounder, '
    'as all Phase 1 patients are hospitalised for surgery by definition.',

    'A within-database confounder-adjustment framework using disease-specific drug proxies '
    'from the same data source provides a transparent and replicable methodology.',

    'The unit of analysis is the prefecture (n=47), not the individual patient; '
    'ecological correlations may not reflect individual-level associations (ecological fallacy).',

    'NDB Open Data lack diagnosis codes, so the neuropathic pain drug proxy captures '
    'all indications for these medications, not chronic postsurgical pain specifically.',
]
...
d1 = (
    f'First, despite Japan\u2019s well-documented cultural stoicism,{cite(5,6)} '
...
d2 = (
    f'Second, the most methodologically important finding is that the dramatic regional variation '
    ...
    f'misinterpreted as differences in CPSP.'
)
doc.add_paragraph(d2)
disc_parts.append(d2)

d3 = (
    f'Third, the positive correlation between Phase 1 (acute) and Phase 2 (chronic, adjusted) '
...
d7a = (
    ...
    'by definition). The within-database confounder adjustment '
    'demonstrated here\u2014using disease-specific drug proxies from the same data '
    'source\u2014provides a replicable framework for other countries with national claims '
    'databases.'
)
doc.add_paragraph(d7a)
disc_parts.append(d7a)
...
doc.add_paragraph(
    ...
    'TO accepts full responsibility for the work, had access to the data, '
...
# ============================================================
# FIGURE LEGENDS (at end per BMJ Open guidelines)
# ============================================================
doc.add_page_break()
...
# SAVE
# ============================================================
outpath = os.path.join(BMJOPEN_DIR, 'BMJOpen_manuscript_EN.docx')
doc.save(outpath)
print(f'\nSaved: {outpath}')

body_total = intro_total + methods_total + results_total + disc_total
print(f'\nBody word count: {body_total} (BMJ Open limit: 4,000)')
print(f'Abstract word count: {abstract_total} (limit: 300)')
print(f'References: {len(ref_list)} (no strict limit)')
