...
def plot_hp_figures(P, L, H, res, bien, out_dir, prefix="hp"):
    """Generate Elsevier-specific figure files without embedded captions.

    Elsevier (Health Policy / Health Policy and Technology) asks that figure
    captions are not printed on the artwork itself, so title=None suppresses
    the figure title while keeping axis labels and panel labels in English."""
    plot_equivalence(
        res["equivalence"], os.path.join(out_dir, f"{prefix}_Figure_1.png"), title=None)
    plot_counts_vs_rates(
        P, L, bien, os.path.join(out_dir, f"{prefix}_Figure_2.png"), title=None,
        colored=True)
    plot_litigation_rate(
        P, L, bien, os.path.join(out_dir, f"{prefix}_Supplementary_Figure_1.png"), title=None)
    plot_physician_index(
        P, bien, os.path.join(out_dir, f"{prefix}_Supplementary_Figure_2.png"), title=None)
    print(f"wrote {prefix}_* figure files to", out_dir)
