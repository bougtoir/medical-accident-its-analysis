#!/usr/bin/env python3
"""Build a Social Science & Medicine (Elsevier) submission package for the
...
Outputs (all derived from results/reanalysis_results.json and data_primary/):
  - manuscript/ssm_manuscript_en.docx   anonymised main manuscript
  - manuscript/ssm_title_page.docx      title page with author info
  - manuscript/ssm_cover_letter.docx    cover letter addressed to SSM
  - manuscript/ssm_highlights.docx      3-5 short highlights
  - manuscript/ssm_supplementary.docx     supplementary figures & tables
  - manuscript/ssm_figures.pptx           editable main figure slides
  - manuscript/ssm_supplementary_figures.pptx editable supplementary slides
  - output/ha_Figure_*.png              main figure files (reused)
  - output/ha_Supplementary_Figure_*.png supplementary figure files (reused)
  - output/ssm_submission.zip            bundled submission package
...
SSM_TITLE = (
    "Malpractice litigation risk and physician specialty supply in Japan: a national panel study"
)
...
def _ssm_get_cite_year(ref: str, key: str) -> str:
    """Return the publication year with a Harvard-style a/b/c suffix when needed."""
    return _ssm_get_year(ref) + _SSM_YEAR_SUFFIX.get(key, "")


def _ssm_get_year(ref: str) -> str:
...
_SSM_YEAR_SUFFIX = _ssm_build_year_suffixes(ha.REFS)


def _ssm_has_initials(token: str) -> bool:
...
def _ssm_split_authors(author_part: str):
    ...
    author_like = sum(1 for t in raw if _ssm_has_initials(t))
    if et_al or author_like >= 2:
        return raw
    return [author_part.strip()]


def _ssm_parse_author_token(token: str) -> str:
...
def _ssm_format_author_list(author_part: str) -> str:
    """Format an author string for the reference list in Harvard style."""
    tokens = _ssm_split_authors(author_part)
    parsed = [_ssm_parse_author_token(t) for t in tokens if t.strip()]
    if not parsed:
        return author_part.strip()
    if len(parsed) == 1:
        return parsed[0]
...
def _ssm_in_text_author(ref: str, key: str) -> str:
    ...
    if key in _SSM_CITE_SHORT:
        return _SSM_CITE_SHORT[key]
    first = ref.split(". ", 1)[0]
    tokens = _ssm_split_authors(first)
    et_al = any("et al" in t.lower() for t in tokens)
    parsed = [_ssm_parse_author_token(t) for t in tokens]
...
def _ssm_ensure_title_end(title: str) -> str:
...
def _ssm_article_title(title: str) -> str:
    """Return a sentence-case article title with no trailing terminal punctuation."""
    title = _ssm_sentence_case(title.strip()).strip()
    return title.rstrip(".")


def _ssm_sentence_case(title: str) -> str:
    """Convert an article title to sentence case for SSM Harvard references.
...
def _ssm_fmt_journal_tail(tail: str) -> str:
    """Format a Vancouver year;volume(issue):pages tail for SSM Harvard style.

    SSM expects: volume(issue): pages (no space before issue, colon before pages).
    """
...
def _ssm_fmt_book_tail(tail: str, year: str) -> str:
    """Format a report/book tail (place:publisher;year url) for SSM Harvard style."""
    ...
    if not core:
        ...
        return ""
    # SSM published style uses "Place: Publisher."
    if ":" in core:
...
def _ssm_harvard_segments(ref: str, key: str):
    """Convert a Vancouver-style reference string to SSM Harvard-style text runs.
    ...
    if key == "angrist":
        author = _ssm_format_author_list("Angrist JD, Pischke JS")
        return [
    ...
    base_year = _ssm_get_year(ref)
    year = base_year + _SSM_YEAR_SUFFIX.get(key, "")
    ...
    author_str = _ssm_format_author_list(author_part)
    prefix = f"{author_str} ({year})"
    if len(parts) == 1:
        return [(f"{prefix}.", False, False)]
    if len(parts) == 2:
        title = _ssm_ensure_title_end(parts[1].strip())
    ...
    if re.search(r"^\d{4};", tail):
        title = _ssm_article_title(raw_title)
        journal_tail = _ssm_fmt_journal_tail(tail)
        return [
    ...
    title = _ssm_ensure_title_end(_ssm_sentence_case(raw_title))
    rest = _ssm_fmt_book_tail(tail, base_year)
    return [
        (f"{prefix}, ", False, False),
        (title, True, False),
        (f" {rest}", False, False),
...
def build_manuscript(inline=False):
    ...
    # Cambridge/SSM submission guidance (figures and tables should not be
    ...
    # Harvard-style citation state for SSM
    _cite_order = []
    _first_cite = set()
    ...
    def _cite(text: str) -> str:
        def _repl(match):
            ...
            for k in keys:
                ...
                ref = ha.REFS[k]
                if k in _SSM_CITE_FIRST and k not in _first_cite:
                    author = _SSM_CITE_FIRST[k]
                else:
                    author = _ssm_in_text_author(ref, k)
                _first_cite.add(k)
                year = _ssm_get_cite_year(ref, k)
    ...
    if abstract_wc > 250:
        raise SystemExit(f"Abstract is {abstract_wc} words; SSM limit is 250")
    ...
    ha._add_runs(
        kw,
        "malpractice litigation; physician workforce; specialty maldistribution; "
        "health policy; Japan; equivalence testing",
    )
    ...
    b(
        doc,
        "Specialty maldistribution is a persistent health-workforce problem across high-income "
        "health systems. High-acuity fields such as surgery, obstetrics and gynaecology, paediatrics "
        "and anaesthesiology are widely perceived as understaffed, with large regional and "
        "specialty-level imbalances.{maldist} A recurring policy intuition -- and a common media "
        "narrative -- is that the fear of malpractice litigation and medical safety incidents pushes "
    ...
    b(
        ...
        "whether that anxiety translates into aggregate workforce shifts in a health system that "
        "structures the returns to and costs of specialty choice.",
    )
    b(
        doc,
        ...
        "Labour and Welfare publishes biennial physician counts and annual hospital facility "
        "counts, and the country shares the fee-for-service pressures seen in other high-income "
        "countries.{court,phys,facil} It also introduced the Japan Obstetric Compensation System "
        "for Cerebral Palsy (JOCS-CP) in 2009, a no-fault scheme intended partly to address "
        "obstetric workforce concerns.{jocscp,hasegawa2016} Surveys of residents in Japan, Korea "
        "and Taiwan report litigation as a negative factor, but workload, lifestyle and "
        "professional interest dominate specialty choice.{lin2022}",
    )
    b(
        doc,
        ...
        "freedom of any lag-based method. We address these pitfalls with an analytical framework "
        "that uses rates rather than counts, uses only measured biennial physician observations, "
        "and applies pre-specified equivalence testing, which can provide positive evidence for the "
        "absence of a meaningful effect rather than merely failing to reject a null.{lakens,schuir}",
    )
    # Methods
    ...
    b(
        ...
        "planning. For health policy and medical sociology, this is an important distinction: a widely "
        "discussed risk can be salient without being a reliable policy lever. The policy question is not "
        "whether physicians worry about litigation, but whether a policy that reduces litigation risk would "
        "materially change aggregate specialty supply. Our evidence suggests it would not.",
    )
    h(doc, "Interpretation in institutional context", level=2)
    b(
        doc,
        "Several mechanisms are consistent with this finding. First, litigation risk may affect clinical "
        "behaviour on the intensive margin (defensive medicine) rather than the extensive margin "
    ...
    h(doc, "Legal and international context", level=2)
    b(
        doc,
    ...
    b(
        doc,
        f"This is an ecological, specialty-level analysis and cannot establish individual-level causality. We do not directly observe individual physicians' subjective risk perceptions, so the interpretation of the null result in terms of availability, loss aversion and status-quo bias is theoretically informed rather than directly tested. The {N} specialties "
    ...
    b(
        ...
        f"these national data, and the physician effect was statistically equivalent to null within a small margin. Malpractice "
        f"litigation is not a reliable policy lever for correcting specialty maldistribution in this setting. Policymakers may more "
        f"productively target structural incentives, especially no-fault compensation and payment design, than rely on the assumption "
        f"that reducing litigation will retain physicians in high-risk specialties. The transparent, reproducible sensitivity-analysis "
        f"framework used here is exportable to other healthcare workforce-policy questions. More generally, the results caution "
        f"against treating salient risks as policy instruments when the underlying institutional context preserves strong structural "
        f"incentives.",
    )
    ...
    # Declarations (Cambridge / SSM back-matter requirement)
    ...
    out = os.path.join(BASE, "ssm_manuscript_en.docx")
...
def build_title_page(main_word_count, total_word_count):
    ...
    lines = [
        ...
        "Target journal: Social Science & Medicine (Elsevier)",
        ...
        "CRediT author statement: Onishi Tatsuki - Conceptualization, Data curation, Formal analysis, Methodology, Software, Writing - original draft, Writing - review and editing.",
    ]
    for line in lines:
        p = doc.add_paragraph()
    ...
    out = os.path.join(BASE, "ssm_title_page.docx")
    ha.normalize_docx(doc)
    doc.save(out)
    print("wrote", out)
...
def build_highlights():
    highlights = [
        "No litigation-risk effect on physician or hospital supply growth in Japan.",
        "Rate-based, measured-only design avoids size confounding and interpolation bias.",
        "Equivalence tests bound litigation effects below policy-relevant margins.",
        "Structural incentives outweigh perceived litigation risk in career choices.",
        "No-fault compensation and payment design are more promising policy levers.",
    ]
    for h_item in highlights:
        if len(h_item) > 85:
            raise SystemExit(f"Highlight exceeds 85 characters ({len(h_item)}): {h_item}")
    ...
    out = os.path.join(BASE, "ssm_highlights.docx")
    ha.normalize_docx(doc)
    doc.save(out)
    print("wrote", out)
...
def build_cover_letter():
    ...
    p = doc.add_paragraph()
    p.add_run("Dear Professor Elliott,").font.size = Pt(12)
    p.runs[0].font.name = "Times New Roman"

    paragraphs = [
        f'We submit an original research article, "{SSM_TITLE}", for consideration by Social Science & Medicine.',
        "Social Science & Medicine provides an international interdisciplinary forum for research on health, and our study informs a central health-policy and workforce-planning question: whether malpractice litigation risk shapes the distribution of physicians across specialties. Using national administrative data from Japan, we find no association between specialty-level litigation risk and physician or hospital supply, and we bound any effect within a small equivalence margin.",
        "The paper makes three contributions. First, it shows how two common observational fallacies -- size confounding in raw administrative counts and interpolation of sparse panel data -- can distort evidence on litigation-workforce relationships. Second, it combines fixed-effects panel methods, equivalence testing, cluster block-bootstrap and power diagnostics to produce an informative null result rather than a mere failure to reject the null. Third, it interprets the null through the lens of institutional and behavioural mechanisms -- availability, loss aversion and status-quo bias -- that explain why perceived risk can be high while aggregate workforce response remains negligible.",
        "The national administrative data come from Japan, a setting that provides a complete, long-running test case. The analysis is fully reproducible from openly available primary files and code in the project repository ("
        f"{COVER_REPO_PLACEHOLDER}). We believe the manuscript will be of interest to health economists, health-policy scholars, and social-science researchers concerned with how risk, institutions and incentives shape workforce behaviour. We suggest the Health Policy office as the most appropriate handling office.",
    ...
    out = os.path.join(BASE, "ssm_cover_letter.docx")
    ha.normalize_docx(doc)
    doc.save(out)
    print("wrote", out)
...
def build_supplementary():
    """Build the Healthcare Analytics supplementary file and copy it to a SSM-named file."""
    ...
    dst = os.path.join(BASE, "ssm_supplementary.docx")
    shutil.copyfile(src, dst)
    print("wrote", dst)


def build_figure_pptx():
    """Build the Healthcare Analytics editable figure PPTX files and copy them to SSM-named files."""
    # Always regenerate so the embedded figures match the latest output/ha_*.png files.
    ha.build_figure_pptx()
    pairs = [
        ("ha_figures.pptx", "ssm_figures.pptx"),
        ("ha_supplementary_figures.pptx", "ssm_supplementary_figures.pptx"),
    ]
...
def create_submission_zip():
    """Bundle the SSM submission files."""
    zip_path = os.path.join(OUT, "ssm_submission.zip")
    file_map = [
        (os.path.join(BASE, "ssm_manuscript_en.docx"), "ssm_manuscript_en.docx"),
        (os.path.join(BASE, "ssm_title_page.docx"), "ssm_title_page.docx"),
        (os.path.join(BASE, "ssm_cover_letter.docx"), "ssm_cover_letter.docx"),
        (os.path.join(BASE, "ssm_highlights.docx"), "ssm_highlights.docx"),
        (os.path.join(BASE, "ssm_supplementary.docx"), "ssm_supplementary.docx"),
        (os.path.join(BASE, "ssm_figures.pptx"), "ssm_figures.pptx"),
        (os.path.join(BASE, "ssm_supplementary_figures.pptx"), "ssm_supplementary_figures.pptx"),
    ]
    figure_names = [
...
def create_figures_upload_zip():
    ...
    zip_path = os.path.join(OUT, "ssm_figures_for_upload.zip")
    figure_names = [
        "Figure_1", "Figure_2", "Figure_3", "Figure_4",
        "Supplementary_Figure_1", "Supplementary_Figure_2", "Supplementary_Figure_3",
    ]
...
def main():
    ...
    build_supplementary()
    build_figure_pptx()
    # Sanitise generated Office files so no multibyte characters remain in XML.
    for fn in [
        "ssm_manuscript_en.docx",
        "ssm_title_page.docx",
        "ssm_cover_letter.docx",
        "ssm_highlights.docx",
        "ssm_supplementary.docx",
        "ha_supplementary.docx",
        "ssm_figures.pptx",
        "ssm_supplementary_figures.pptx",
        "ha_figures.pptx",
        "ha_supplementary_figures.pptx",
    ]:
