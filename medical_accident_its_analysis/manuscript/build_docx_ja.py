...
add_figure(
    ...
    "図2. 12主要診療科の医師数と医療事故調査制度報告件数の分割時系列プロット。分割回帰の適合線を示す。垂直破線は介入年（事故報告ピーク年）を示す。"
)
...
add_figure(
    ...
    "図3. 相互相関分析による推定リードタイム（年）のヒートマップ。行は診療科、列は事故定義・アウトカムの組み合わせを示す。濃い色はより強い逆相関を示す。正のラグ値は事故件数の変化がアウトカムの変化に先行することを示す。"
)
...
add_figure(
    ...
    "図4. 12主要診療科の施設数と医療事故調査制度報告件数の分割時系列プロット。分割回帰の適合線を示す。"
)
...
add_figure(
    ...
    "図5. 主要診療科の予測医師数（2025〜2034年）。95%予測区間付き線形トレンド外挿に基づく。"
)

# --- 図6: 施設数予測 ---
add_figure(
    os.path.join(OUTPUT_DIR, "forecast_facilities.png"),
    "図6. 主要診療科の予測施設数（2025〜2034年）。95%予測区間付き線形トレンド外挿に基づく。"
)

# --- 図7: 専攻医数予測 ---
add_figure(
    os.path.join(OUTPUT_DIR, "forecast_trainees.png"),
    "図7. 主要診療科の予測専攻医登録数（2025〜2034年）。95%予測区間付き線形トレンド外挿に基づく。"
)
