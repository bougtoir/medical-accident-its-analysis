...
ABSTRACT = (
    ...
    "dynamics, we show that the PK toolkit is a genuinely useful set of "
    ...
    "structure survives 1PN precession and tidal drag, confirming "
    "robustness. The mapping is bidirectional \u2014 celestial-mechanics "
...
DISCUSSION = [
    ("We set out to test whether the pharmacokinetic toolkit, imported "
     "wholesale, is genuinely useful for the statistical three-body problem. "
     "It is. The compartment model reproduces the multi-exponential lifetime "
     "law and hands back the mean residence time in closed form; the "
     "Michaelis\u2013Menten extension parametrises sticky chaos with one or "
     "two constants; and population-PK regression compresses the entire "
     "mass-ratio dependence into an allometric scaling law. Crucially, these "
     "are not restatements of known statistical theory but transferable "
     "estimators built on top of it: the PK MRT tracks the empirical mean "
     "lifetime across 64 configurations, and the multi-exponential survival "
     "coincides with the ergodic prediction while a single flux timescale "
     "does not."),

    ("It is worth stating plainly what the imported toolkit newly enables, as "
     "distinct from what statistical three-body theory already provided. "
     "Flux-based and random-walk treatments already deliver lifetime "
     "distributions, branching ratios and ejection statistics,{5,6} and we do "
     "not claim new dynamics; the PK reading reproduces them (Section 3.7). "
     "What it adds are practical capabilities that were awkward or absent "
     "before: (i) a single closed-form scalar, the mean residence time "
     "MRT = \u22121^T A^{\u22121} P0, that feeds directly into merger-rate and "
     "environment-timescale calculations without re-running Monte-Carlo "
     "integrations; (ii) a population/allometric regression that compresses "
     "the whole mass-spectrum dependence into a re-usable scaling law with "
     "quantified covariate importance (variance-inflation factors and an "
     "LMG/Shapley decomposition), replacing per-spectrum ad hoc fitting; "
     "(iii) a one-to-two-parameter Michaelis\u2013Menten description that "
     "bridges the ergodic and sticky-chaos regimes continuously rather than "
     "fitting a bare power-law tail; and (iv) a genuinely new direction of "
     "travel \u2014 exporting celestial-mechanics stability analysis back to "
     "pharmacology (Section 3.4), which no prior three-body treatment "
     "attempts. These are gains in transferability and workflow, not in "
     "fundamental physics, and that is precisely the point."),

    ("We stress that this is more than a mathematical curiosity. The value of "
     "the exercise is the disciplined, two-way exchange of concepts: a mature "
     "clinical science with decades of validated estimators, diagnostics and "
     "software (residence-time calculus, mixed-effects population modelling, "
     "saturable kinetics, dose\u2013response analysis) is shown to carry real, "
     "quantitative predictive weight in an entirely different physical domain, "
     "and in return that domain contributes geometric stability tools to "
     "pharmacology. Because the bridge rests only on the shared "
     "continuous-time Markov structure of multi-state first-passage "
     "processes, importing a ready-made, battle-tested toolkit \u2014 rather "
     "than rebuilding equivalent machinery from scratch \u2014 is a concrete "
     "methodological economy, and it suggests that such cross-disciplinary "
     "concept transfer is a productive research strategy in its own right, "
     "not merely an aesthetic analogy."),

    ("The astrophysical payoff is concrete. The same closed-form outputs "
     "yield Peters-inspiral merger fractions and a large eccentricity boost "
     "of the GW merger rate, a dose\u2013response predictor for which body is "
     "ejected (giving mass segregation once convolved with a realistic mass "
     "function), the eccentricity and kick distributions of the products, "
     "and environment-specific resonance-versus-encounter timescales for "
     "globular clusters, nuclear star clusters, and AGN discs. Because these "
     "follow from a handful of fitted rate constants, the framework can be "
     "re-tuned to a new mass spectrum or environment cheaply, complementing "
     "expensive direct surveys.{13,24,25}"),

    ("The mapping is robust: it survives the move from 2D to 3D, thermal "
     "eccentricities, 2.5PN radiation reaction, 1PN precession, and tidal "
     "drag, with the compartment structure intact throughout. This breadth "
     "matters because it is exactly the regime \u2014 three-dimensional, "
     "eccentric, weakly dissipative \u2014 in which real cluster and disc "
     "encounters occur."),

    ("That the traffic runs both ways is, we argue, the deeper point. "
     "Celestial-mechanics stability theory illuminates TMDD steady states "
     "just as PK illuminates three-body statistics; the correspondence is a "
     "two-way bridge, not a one-directional analogy: the geometric stability "
     "theory of celestial mechanics may inform the design of drugs with "
     "controlled target residence times.{18} Because it rests only on "
     "the continuous-time Markov structure of multi-state first-passage "
     "processes, the same bridge may connect to chemical reaction networks "
    ...
    ("Limitations remain. The radiation-reaction treatment is perturbative "
     ...
     "assumes ergodicity, which the nonlinear extension only partially "
     "repairs at the cost of extra parameters. None of these affects the "
     "central finding that the compartment structure is preserved and "
     "predictive."),
]
...
CONCLUSIONS = [
    ("Motivated by pharmacokinetics, we re-examined the three-body problem "
     ...
     "is a genuinely useful instrument for three-body statistics, "
