...
「一致性相関係数の分解により、ゼロ校正はオフセットのみを補正し
  ゲインは補正しないことが明らかに：動脈圧モニターバリデーションの
  シミュレーション枠組み」
...
run_en = title_en.add_run(
    ...
    'Corrects Offset but Not Gain: A Simulation-Based Framework for\n'
    'Arterial Pressure Monitor Validation'
)
...
add_para('ランニングタイトル：CCC分解による動脈圧バリデーション', italic=True,
...
add_para(
    ...
    '3つの工学的解決策\u2014カテーテル先端MEMSセンサ、気圧補償、自己校正MEMS\u2014を特定した。'
)
...
add_para(
    ...
    '脈圧の正確性が暗黙的なゲイン検証として機能することを示した。脈圧はDCオフセットから独立だが、'
    'センサゲインに比例するためである。'
)
...
add_para(
    ...
    '血管内カテーテル先端と接続されている。この構造は3つの系統的DCオフセット源を導入する：'
...
add_para(
    ...
    '（Critchley et al 2010, 2011）を中心としている。これらのツールはバイアス、一致の限界、'
...
add_heading_styled('2.1. 動脈圧のAC・DC成分分解', level=2)
...
add_para(
    ...
    '生理的・測定的変動をシミュレートするため、すべてのシナリオにガウスノイズ（SD = 3.5 mmHg）を加えた。'
...
add_para(
    '図2に4つのシミュレーションシナリオのコンコーダンスプロットを示す。'
...
add_para(
    '重要な比較はシナリオBとCの間にある：Bland\u2013Altman解析（図5、図8）はいずれの場合も'
...
add_para(
    '図6はこれらの関係を定量化する。パネルAはゲインエラーとDCオフセットの関数としてのC\u2082を示し、'
...
add_para(
    '図7はPP\u2013ゲイン関係の定量的実証を提供する。正しいゲイン（パネルA）、ゲインエラー（パネルB）、'
...
add_para(
    ...
    '商用または商用化間近の製品に個別にすでに存在している（Hasenkamp et al 2012, Song et al 2020, '
    'Millar 2026, 2026b, Kang et al 2022）。これらを単一の臨床用動脈圧モニタリングシステムに統合する'
    'ことは、科学的課題ではなく工学的課題である。'
)
...
add_para(
    ...
    'いない（FDA 2026）。公表されたバリデーション研究は圧倒的にBland\u2013Altman解析とpercentage errorに'
    '依拠している（Joosten et al 2017, Bland and Altman 1986）。しかし、ゼロ校正後の機器の'
...
add_para(
    ...
    'NICOM/Starlingのバイオリアクタンス位相基準は位相\u2013一回拍出量変換のベースラインオフセットを提供する'
    '（Squara et al 2007）。FloTracの動脈圧波形較正は平均圧オフセットを調整する（Manecke 2005）。'
    'いずれの場合も、自動校正ルーチンはu（ロケーションシフト）を処理するが、v（スケールシフト）'
    '\u2014すなわち圧力\u2192COまたはインピーダンス\u2192SVの変換ゲイン\u2014は補正しない。'
)
...
add_figure(
    ...
    'vは変化しない。点A\u2013Dは図2のシナリオに対応する。金色の星は理想（u = 0, v = 1, C\u2082 = 1.0）'
...
add_figure(
    ...
    '図6. 感度分析。(A) ゲインエラー（%）とDCオフセット（mmHg）の関数としてのC\u2082値：'
