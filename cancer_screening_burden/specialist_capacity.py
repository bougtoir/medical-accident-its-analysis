...
def compute_specialist_capacity_impact(
    ...
    for _, c in mapping.iterrows():
        ...
        sim_row = sim.iloc[0]
        # specialist_total_visits in the simulation is per 100k screened.
        fp_visits_per_specialist = sim_row["specialist_total_visits"] / spec_per_100k
        ...
        rows.append(
            {
                ...
                "fp_specialist_visits_per_100k": sim_row["specialist_total_visits"],
...
def build_specialist_capacity_docx(df: pd.DataFrame, inputs: Dict[str, float], path: Path) -> None:
    ...
    doc.add_paragraph(
        "Note: FP visits are the additional specialist visits generated by a 100,000-person "
        "DTC MCED screening wave at the stated follow-up rate. The 'baseline cases per specialist' "
        "uses the MHLW Patient Survey 2023 total patient stock (総患者数) for each cancer."
    )
    doc.save(path)
