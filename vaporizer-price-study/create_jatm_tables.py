"""
Generate standalone editable .docx files for each table (JATM submission).
Tables are format-neutral; identical to JCA version with JATM output filenames.
"""
import os
import pandas as pd
import numpy as np
...
def write_table1(df):
    ...
    path = os.path.join(out_dir, 'jatm_table1.docx')
    doc.save(path)
...
def write_table2(df):
    ...
    path = os.path.join(out_dir, 'jatm_table2.docx')
    doc.save(path)
...
def write_table_s1(df):
    ...
    path = os.path.join(out_dir, 'jatm_table_s1.docx')
    doc.save(path)
