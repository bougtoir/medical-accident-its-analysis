...
The pipeline uses the pre-extracted cohort and sampled works in `data/cohort/` so it can be reproduced without OpenAlex API access. To re-extract the underlying works, run `python src/openalex_client.py` before `reproduce.sh`; this uses the on-disk cache in `data/cache/`.
...
- Endogenous PI-driven inflow: `I(P_D) = I0 + r · P_D` with `r` capped at a safety factor (`r / r_critical`) below 1 to keep the linear system stable.
