...
def create_manuscript():
    ...
    # --- Section 7: Preliminary Evidence from GEO ---
    add_heading(doc, "Preliminary Evidence: Academic Calendar Confounds in Public Databases", level=2)

    p = doc.add_paragraph()
    set_paragraph_format(p, line_spacing=1.5)
    add_superscript_refs(p,
        "To begin testing whether seasonal patterns in PSC research reflect biology or "
        "institutional rhythms, we analyzed 6,101 PSC differentiation datasets from NCBI "
        "GEO{34} (2001–2026). Country of origin was determined for 3,195 records. The full "
        "dataset showed significant monthly non-uniformity (χ²=32.3, p=0.0007) with a March "
        "peak—superficially consistent with a spring photoperiod effect in the Northern "
        "Hemisphere. However, a natural experiment exploiting geographic variation in "
        "academic year starts revealed a very different picture."
    )

    p = doc.add_paragraph()
    set_paragraph_format(p, line_spacing=1.5)
    add_superscript_refs(p,
        "We grouped countries by academic/fiscal year start month: Japan and South Korea "
        "(March/April start, n=250), USA/Europe/China (September/October start, n=2,825), "
        "and Australia/New Zealand/Brazil (January/February start, Southern Hemisphere, "
        "n=83). Using Pearson's χ² and circular Rayleigh tests,{35} we found that only "
        "Japan/South Korea showed statistically significant seasonality (χ²=32.8, p=0.0006, "
        "Rayleigh R=0.172) with a pronounced March peak—perfectly aligned with Japan's "
        "fiscal year-end. The much larger September-start group (n=2,825) showed no "
        "significant seasonal pattern (p=0.154). Figure 3 presents these results."
    )

    # Figure 3 - natural experiment
    fig3_path = os.path.join(OUTPUT_DIR, "natural_experiment_figure.png")
    if os.path.exists(fig3_path):
        doc.add_picture(fig3_path, width=Inches(6.0))
    else:
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = p.add_run("[FIGURE 3 — see natural_experiment_figure.png]")
        run.bold = True
        run.font.color.rgb = RGBColor(150, 150, 150)
        set_paragraph_format(p, space_before=Pt(12), space_after=Pt(6))
    ...
    set_paragraph_format(p, space_after=Pt(12))
    run = p.add_run(
        "Figure 3. Natural experiment using geographic variation in academic year starts. "
    )
    run.bold = True
    run.font.size = Pt(10)
    run = p.add_run(
        ...
        "(B) Country-level heatmap confirms the pattern is driven by Japan. "
        "(C) Circular mean directions illustrate the concentration (Rayleigh R) and timing "
        "of each group's peak. "
        "(D) Statistical summary. *** p<0.001, ns = not significant."
    )
    run.font.size = Pt(10)
    ...
    set_paragraph_format(p, line_spacing=1.5)
    add_superscript_refs(p,
        "A sensitivity analysis confirmed that Japan's contribution, though only 3.3% of "
        "the total dataset, was responsible for the overall March peak signal. Excluding "
        "Japan shifted the peak from March to January and reduced significance from "
        "p=0.0007 to p=0.009. When restricted to records with known country affiliation, "
        "excluding Japan eliminated statistical significance entirely (p=0.128). This "
        "demonstrates that what appeared to be a seasonal biological signal was in fact "
        "an institutional artifact of Japan's fiscal year calendar."
    )

    p = doc.add_paragraph()
    set_paragraph_format(p, line_spacing=1.5)
    add_superscript_refs(p,
        "This finding carries an important methodological lesson: public database metadata "
        "cannot reliably distinguish biological seasonality from institutional rhythms. "
        "GEO submission dates reflect a complex convolution of experiment timing, analysis "
        "duration, writing, peer review, and institutional incentives—with a typical lag of "
        "6–18 months from experiment to data deposition. Any biological seasonality signal "
        "in stem cell differentiation, if present, requires direct experimental evidence "
        "from prospective studies or clinical registries with date-stamped outcomes."
    )
